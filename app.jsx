    ...
    <div style={{width:'100%',maxWidth:'100vw',overflowX:'hidden'}}>
      ...
      <header className="app-header">
        ...
        <nav className="nav-tabs">
          {[['raspored','📋 Raspored'],['ekipa','👥 Ekipa'],['radnici','👷 Radnici'],['spisak','📊 Spisak'],['vozila','🚗 Vozila'],['odjeli','🏕️ Odjeli'],['sihtarica','📄 Šihtarica'],['pregled','🔍 Pregled'],['historija','📜 Historija']].map(([k,l]) =>
...
function EkipaView({ workers, ekipa, setEkipa, selectedDate, setSelectedDate,
                     prevDay, nextDay, yesterday, departments }) {
  const isToday = selectedDate === new Date().toISOString().split('T')[0];

  const emptyDay = () => ({ primci: [], otpremaci: [] });
  const dayData  = ekipa[selectedDate] || emptyDay();

  const save = (newData) => setEkipa(e => ({ ...e, [selectedDate]: newData }));
  ...
  const assignedIds = useMemo(() => {
    ...
    (dayData.otpremaci || []).forEach(o => { if (o.workerId) s.add(o.workerId); });
    return s;
  }, [dayData]);
  ...
  const available = (excludeIds = []) =>
    activeWorkers.filter(w => !assignedIds.has(w.id) || excludeIds.includes(w.id));

  // ── Primci operations ──────────────────────────────────────────────────────
  const addPrimac = () => {
    const row = { id: uid(), primatId: '', radnici: [''], pomocni: [''], note: '' };
    save({ ...dayData, primci: [...(dayData.primci||[]), row] });
  };
  const removePrimac = (id) =>
    save({ ...dayData, primci: dayData.primci.filter(p => p.id !== id) });

  const updatePrimac = (id, patch) =>
    save({ ...dayData, primci: dayData.primci.map(p => p.id === id ? { ...p, ...patch } : p) });

  // Add/remove a slot in radnici or pomocni arrays
  const addSlot = (primacId, field) =>
    updatePrimac(primacId, { [field]: [...(dayData.primci.find(p=>p.id===primacId)[field]||[]), ''] });

  const removeSlot = (primacId, field, idx) => {
    const arr = [...(dayData.primci.find(p=>p.id===primacId)[field]||[])];
    arr.splice(idx, 1);
    updatePrimac(primacId, { [field]: arr });
  };
  ...
  const addOtpremac = () => {
    const row = { id: uid(), workerId: '', deptId: (departments||[])[0]?.id || '', note: '' };
    save({ ...dayData, otpremaci: [...(dayData.otpremaci||[]), row] });
  };
  const removeOtpremac = (id) =>
    save({ ...dayData, otpremaci: dayData.otpremaci.filter(o => o.id !== id) });
  const updateOtpremac = (id, patch) =>
    save({ ...dayData, otpremaci: dayData.otpremaci.map(o => o.id === id ? { ...o, ...patch } : o) });

  // ── Copy from yesterday ───────────────────────────────────────────────────
  const copyFromYesterday = () => {
    const src = ekipa[yesterday];
    if (!src || (!src.primci?.length && !src.otpremaci?.length))
      return alert('Nema unesenih podataka za jučer.');
    if (confirm('Kopirati ekipu od jučer? Trenutni unosi bit će zamijenjeni.'))
      save(JSON.parse(JSON.stringify(src)));
  };

  // ── Colours ───────────────────────────────────────────────────────────────
  const C = {
    panj:    { color:'#2d5a27', pale:'#e8f0e6', border:'#9bc492' },
    radnik:  { color:'#b5620a', pale:'#fdf0e0', border:'#e8c17a' },
    pomocni: { color:'#1a3d5c', pale:'#e4edf5', border:'#9bbfd9' },
    otpr:    { color:'#6b3080', pale:'#f0e8f5', border:'#c4a0d8' },
  };

  // ── Worker picker (inline select) ─────────────────────────────────────────
  const WorkerSelect = ({ value, onChange, extraAllow=[], placeholder='— Odaberi —', style={} }) => {
    ...
    return (
      <select
        ...
        style={{
          ...
          background:'var(--bg)', color: value ? 'var(--text)' : 'var(--text-light)',
          fontFamily:'var(--sans)', width:'100%', ...style,
        }}
      >
        <option value=''>{placeholder}</option>
        {opts.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
        {/* show already-taken worker if currently selected */}
        {value && !opts.find(w=>w.id===value) && (
          <option value={value}>⚠ {wName(value)}</option>
        )}
      </select>
    );
  };
  ...
  return (
    <div>
      ...
      <div className="date-bar">
        <div>
          <div className="date-label">DATUM — DNEVNA EKIPA</div>
          <div style={{display:'flex',alignItems:'center',gap:'0.5rem',marginTop:'0.25rem'}}>
            <button className="date-nav-btn" onClick={prevDay}>‹</button>
            <input type="date" className="date-input" value={selectedDate}
              onChange={e=>setSelectedDate(e.target.value)}/>
        ...
        <div style={{display:'flex',gap:'1.2rem',alignItems:'center',flexWrap:'wrap'}}>
          {[
            {label:'Primači',   val:numPrimaci,   c:C.panj},
            {label:'Radnici/primka', val:numRadnici, c:C.radnik},
            {label:'Pomoćni',   val:numPomocni,   c:C.pomocni},
            {label:'Otpremači', val:numOtpremaci, c:C.otpr},
          ].map(({label,val,c}) => (
            <div key={label} style={{textAlign:'center'}}>
              <div style={{fontFamily:'var(--mono)',fontSize:'1.15rem',fontWeight:700,color:c.color}}>{val}</div>
              <div style={{fontSize:'0.6rem',color:'var(--text-muted)',whiteSpace:'nowrap'}}>{label}</div>
            </div>
          ))}
          <div style={{width:1,height:32,background:'var(--border)'}}/>
          <div style={{textAlign:'center'}}>
            <div style={{fontFamily:'var(--mono)',fontSize:'1.3rem',fontWeight:700,color:'var(--green)'}}>{totalAssigned}</div>
            <div style={{fontSize:'0.6rem',color:'var(--text-muted)'}}>UKUPNO</div>
          </div>
        </div>

        <div style={{marginLeft:'auto',display:'flex',gap:'0.5rem',flexWrap:'wrap'}}>
          <button className="btn btn-secondary btn-sm no-print" onClick={copyFromYesterday}>📋 Kopiraj jučer</button>
          <button className="btn btn-secondary btn-sm no-print" onClick={()=>window.print()}>🖨️ Print</button>
        </div>
      </div>

      {/* ── NERASPOREĐENI warning ───────────────────────────────────────── */}
      {unassigned.length > 0 && (
        <div style={{background:'var(--amber-pale)',border:'1px solid #e8c17a',borderRadius:6,
          padding:'0.55rem 1rem',fontSize:'0.8rem',color:'var(--amber)',
          marginBottom:'1rem',display:'flex',alignItems:'center',gap:'0.5rem',flexWrap:'wrap'}}>
          <span style={{fontWeight:700}}>⚠ Neraspoređeni:</span>
          {unassigned.map(w=>(
            <span key={w.id} style={{background:'white',border:'1px solid #e8c17a',
              borderRadius:12,padding:'0.1rem 0.5rem',fontSize:'0.75rem'}}>{w.name}</span>
          ))}
        </div>
      )}

      {/* ══════════════════════════════════════════════════════════════════
          SEKCIJA 1 — PRIMACI NA ŠUMA PANJU
      ══════════════════════════════════════════════════════════════════ */}
      <div className="card" style={{marginBottom:'1rem'}}>
        ...
        <div style={{
          ...
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:'0.95rem'}}>Primaci na šuma panju</div>
            <div style={{fontSize:'0.7rem',opacity:0.8}}>Svaki primač ima vlastiti tim: radnici u primci + pomoćni radnici</div>
          </div>
          <button
            onClick={addPrimac}
            style={{background:'rgba(255,255,255,0.2)',border:'1px solid rgba(255,255,255,0.4)',
              color:'white',borderRadius:6,padding:'0.35rem 0.8rem',fontSize:'0.8rem',
              fontWeight:600,cursor:'pointer'}}>
            + Dodaj primača
          </button>
        </div>

        {(dayData.primci||[]).length === 0 && (
          <div style={{padding:'2rem',textAlign:'center',color:'var(--text-light)',fontSize:'0.85rem'}}>
            Nema unesenih primača za ovaj dan.
            <br/><button onClick={addPrimac}
              style={{marginTop:'0.75rem',background:C.panj.color,color:'white',border:'none',
                borderRadius:6,padding:'0.4rem 1rem',fontSize:'0.82rem',fontWeight:600,cursor:'pointer'}}>
              + Dodaj prvog primača
            </button>
          </div>
        )}

        {(dayData.primci||[]).map((primac, pi) => (
          <div key={primac.id} style={{
            borderTop: pi===0 ? 'none' : '2px solid var(--bg)',
            padding:'1rem',
          }}>
            {/* ── ROW HEADER: primač number + delete ── */}
            <div style={{display:'flex',alignItems:'center',gap:'0.6rem',marginBottom:'0.8rem'}}>
              <div style={{
                width:28,height:28,borderRadius:'50%',
                background:C.panj.color,color:'white',
                display:'flex',alignItems:'center',justifyContent:'center',
                fontSize:'0.8rem',fontWeight:700,fontFamily:'var(--mono)',flexShrink:0,
              }}>{pi+1}</div>
              <span style={{fontWeight:700,fontSize:'0.85rem',color:C.panj.color}}>Primač #{pi+1}</span>
              <button
                onClick={()=>removePrimac(primac.id)}
                style={{marginLeft:'auto',background:'transparent',border:'1px solid #e0a0a0',
                  color:'var(--red)',borderRadius:5,padding:'0.2rem 0.5rem',
                  fontSize:'0.75rem',cursor:'pointer'}}>
                🗑 Ukloni
              </button>
            </div>

            {/* ── THREE COLUMNS: primač | radnici u primci | pomoćni ── */}
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'0.75rem',alignItems:'start'}}>

              {/* COL 1 — PRIMAČ NA PANJU */}
              <div style={{
                background:C.panj.pale,border:`1px solid ${C.panj.border}`,
                borderRadius:8,padding:'0.65rem',
              }}>
                <div style={{fontSize:'0.67rem',fontWeight:700,color:C.panj.color,
                  textTransform:'uppercase',letterSpacing:'0.08em',
                  fontFamily:'var(--mono)',marginBottom:'0.45rem'}}>
                  🌳 Primač na panju
                </div>
                <WorkerSelect
                  value={primac.primatId}
                  onChange={wId => updatePrimac(primac.id, {primatId:wId})}
                ...
                <input
                  value={primac.note||''}
                  onChange={e=>updatePrimac(primac.id,{note:e.target.value})}
                  placeholder='Napomena...'
                  style={{marginTop:'0.4rem',width:'100%',fontSize:'0.72rem',
                    border:'1px solid var(--border)',borderRadius:4,
                    padding:'0.25rem 0.4rem',fontFamily:'var(--sans)',background:'white'}}
                />
              </div>

              {/* COL 2 — RADNICI U PRIMCI */}
              <div style={{
                background:C.radnik.pale,border:`1px solid ${C.radnik.border}`,
                borderRadius:8,padding:'0.65rem',
              }}>
                <div style={{display:'flex',alignItems:'center',marginBottom:'0.45rem',gap:'0.4rem'}}>
                  <div style={{fontSize:'0.67rem',fontWeight:700,color:C.radnik.color,
                    textTransform:'uppercase',letterSpacing:'0.08em',
                    fontFamily:'var(--mono)',flex:1}}>
                    📋 Radnici u primci
                  </div>
                  <button onClick={()=>addSlot(primac.id,'radnici')}
                    style={{background:C.radnik.color,color:'white',border:'none',
                      borderRadius:4,padding:'0.15rem 0.45rem',fontSize:'0.7rem',
                      fontWeight:700,cursor:'pointer'}}>+</button>
                </div>
                {(primac.radnici||['']).map((wId,ri)=>(
                  <div key={ri} style={{display:'flex',gap:'0.3rem',marginBottom:'0.3rem',alignItems:'center'}}>
                    <WorkerSelect
                      value={wId}
                    ...
                    />
                    {(primac.radnici||[]).length > 1 && (
                      <button onClick={()=>removeSlot(primac.id,'radnici',ri)}
                        style={{background:'transparent',border:'none',
                          color:'#c0392b',cursor:'pointer',fontSize:'0.9rem',
                          padding:'0 0.2rem',lineHeight:1,flexShrink:0}}>✕</button>
                    )}
                  </div>
                ))}
              </div>

              {/* COL 3 — POMOĆNI RADNICI */}
              <div style={{
                background:C.pomocni.pale,border:`1px solid ${C.pomocni.border}`,
                borderRadius:8,padding:'0.65rem',
              }}>
                <div style={{display:'flex',alignItems:'center',marginBottom:'0.45rem',gap:'0.4rem'}}>
                  <div style={{fontSize:'0.67rem',fontWeight:700,color:C.pomocni.color,
                    textTransform:'uppercase',letterSpacing:'0.08em',
                    fontFamily:'var(--mono)',flex:1}}>
                    🔧 Pomoćni radnici
                  </div>
                  <button onClick={()=>addSlot(primac.id,'pomocni')}
                    style={{background:C.pomocni.color,color:'white',border:'none',
                      borderRadius:4,padding:'0.15rem 0.45rem',fontSize:'0.7rem',
                      fontWeight:700,cursor:'pointer'}}>+</button>
                </div>
                {(primac.pomocni||['']).map((wId,hi)=>(
                  <div key={hi} style={{display:'flex',gap:'0.3rem',marginBottom:'0.3rem',alignItems:'center'}}>
                    <WorkerSelect
                      value={wId}
                      ...
                      style={{flex:1,background:'white'}}
                    />
                    {(primac.pomocni||[]).length > 1 && (
                      <button onClick={()=>removeSlot(primac.id,'pomocni',hi)}
                        style={{background:'transparent',border:'none',
                          color:'#c0392b',cursor:'pointer',fontSize:'0.9rem',
                          padding:'0 0.2rem',lineHeight:1,flexShrink:0}}>✕</button>
                    )}
      ...
      <div className="card" style={{marginBottom:'1.5rem'}}>
        <div style={{
          ...
          <span style={{fontSize:'1.4rem'}}>🚛</span>
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:'0.95rem'}}>Otpremači</div>
            <div style={{fontSize:'0.7rem',opacity:0.8}}>Raspoređuju se na odjel / radilište</div>
          </div>
          <button onClick={addOtpremac}
            style={{background:'rgba(255,255,255,0.2)',border:'1px solid rgba(255,255,255,0.4)',
              color:'white',borderRadius:6,padding:'0.35rem 0.8rem',fontSize:'0.8rem',
              fontWeight:600,cursor:'pointer'}}>
            + Dodaj otpremača
        ...
        {(dayData.otpremaci||[]).length === 0 ? (
          <div style={{padding:'1.5rem',textAlign:'center',color:'var(--text-light)',fontSize:'0.85rem'}}>
            Nema unesenih otpremača za ovaj dan.
          </div>
        ) : (
          <div style={{padding:'0.75rem',display:'flex',flexDirection:'column',gap:'0.5rem'}}>
            {(dayData.otpremaci||[]).map((otr, oi) => (
              <div key={otr.id} style={{
                display:'grid',
                gridTemplateColumns:'auto 1fr 1fr auto',
                gap:'0.6rem',alignItems:'center',
                background:C.otpr.pale,border:`1px solid ${C.otpr.border}`,
                borderRadius:8,padding:'0.6rem 0.8rem',
              }}>
                {/* index */}
                <div style={{
                  width:26,height:26,borderRadius:'50%',
                  background:C.otpr.color,color:'white',
                  display:'flex',alignItems:'center',justifyContent:'center',
                  fontSize:'0.75rem',fontWeight:700,fontFamily:'var(--mono)',flexShrink:0,
                }}>{oi+1}</div>

                {/* worker */}
                <div>
                  <div style={{fontSize:'0.65rem',fontWeight:700,color:C.otpr.color,
                    textTransform:'uppercase',letterSpacing:'0.06em',
                    fontFamily:'var(--mono)',marginBottom:'0.25rem'}}>Otpremač</div>
                  <WorkerSelect
                    value={otr.workerId}
                    onChange={wId=>updateOtpremac(otr.id,{workerId:wId})}
                ...
                <div>
                  <div style={{fontSize:'0.65rem',fontWeight:700,color:C.otpr.color,
                    textTransform:'uppercase',letterSpacing:'0.06em',
                    fontFamily:'var(--mono)',marginBottom:'0.25rem'}}>Odjel / Radilište</div>
                  <select
                    value={otr.deptId||''}
                    onChange={e=>updateOtpremac(otr.id,{deptId:e.target.value})}
                    style={{border:'1px solid var(--border)',borderRadius:5,
                      padding:'0.38rem 0.6rem',fontSize:'0.82rem',
                      background:'white',fontFamily:'var(--sans)',width:'100%'}}>
                ...
                {/* delete */}
                <button onClick={()=>removeOtpremac(otr.id)}
                  style={{background:'transparent',border:'1px solid #e0a0a0',
                    color:'var(--red)',borderRadius:5,padding:'0.3rem 0.5rem',
                    fontSize:'0.75rem',cursor:'pointer',alignSelf:'flex-end'}}>
                  🗑
                </button>
              </div>
            ))}
      ...
      <div className="card">
        <div className="card-header">
          <span>📊</span>
          <div className="card-title">Pregled ekipe — {fmtDate(selectedDate)}</div>
        </div>
        <table className="schedule-table">
          <thead>
            <tr>
          ...
          <tbody>
            {(dayData.primci||[]).flatMap((p,pi) => {
              const rows = [];
              // primač row
              rows.push(
                <tr key={`p${pi}-prim`}>
                  <td style={{fontFamily:'var(--mono)',color:'var(--text-muted)',fontSize:'0.72rem'}}>{pi+1}</td>
                  <td><span style={{display:'inline-flex',alignItems:'center',gap:'0.25rem',
                    background:C.panj.pale,color:C.panj.color,border:`1px solid ${C.panj.border}`,
                    borderRadius:4,padding:'0.15rem 0.5rem',fontSize:'0.68rem',fontWeight:700,fontFamily:'var(--mono)'}}>
                    🌳 Primač/panj</span></td>
                  <td style={{fontWeight:700}}>{p.primatId ? wName(p.primatId) : <span style={{color:'var(--text-light)'}}>—</span>}</td>
                  <td style={{fontSize:'0.78rem',color:'var(--text-muted)'}}>
                    <div>📋 {(p.radnici||[]).filter(Boolean).map(wName).join(', ') || '—'}</div>
                    <div>🔧 {(p.pomocni||[]).filter(Boolean).map(wName).join(', ') || '—'}</div>
                  </td>
                  <td style={{fontSize:'0.78rem',color:'var(--text-muted)'}}>{p.note||'—'}</td>
                </tr>
              );
              return rows;
            })}
            {(dayData.otpremaci||[]).map((o,oi) => (
              <tr key={`o${oi}`}>
                <td style={{fontFamily:'var(--mono)',color:'var(--text-muted)',fontSize:'0.72rem'}}>{oi+1}</td>
                <td><span style={{display:'inline-flex',alignItems:'center',gap:'0.25rem',
                  background:C.otpr.pale,color:C.otpr.color,border:`1px solid ${C.otpr.border}`,
                  borderRadius:4,padding:'0.15rem 0.5rem',fontSize:'0.68rem',fontWeight:700,fontFamily:'var(--mono)'}}>
                  🚛 Otpremač</span></td>
                <td style={{fontWeight:700}}>{o.workerId ? wName(o.workerId) : <span style={{color:'var(--text-light)'}}>—</span>}</td>
                <td style={{fontSize:'0.78rem',color:'var(--text-muted)'}}>{o.deptId ? dName(o.deptId) : '—'}</td>
                <td style={{fontSize:'0.78rem',color:'var(--text-muted)'}}>{o.note||'—'}</td>
              </tr>
            ))}
            {totalAssigned === 0 && (
              <tr><td colSpan={5} style={{textAlign:'center',padding:'2rem',color:'var(--text-light)'}}>
        ...
        <div className="card" style={{marginBottom:'1rem',padding:'1rem'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))',gap:'0.75rem'}}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" type="number" min="1" max="60" value={form.brojMjesta} onChange={e=>setForm(f=>({...f,brojMjesta:parseInt(e.target.value)||1}))} />
            </div>
            <div className="form-group">
              <label className="form-label">Status</label>
...
// ─── ŠIHTARICA VIEW ──────────────────────────────────────────────────────────
function SihtaricaView({ schedules, workers, departments, godisnji, setGodisnji, wName, dName }) {
  const now = new Date();
  ...
  const workerStats = useMemo(() => {
    return workers.map(w => {
      let radnih = 0, odsutnih = 0, vikenda = 0, praznih = 0;
      const odsutTypes = {};
      days.forEach(d => {
      ...
      return { ...w, radnih, odsutnih, vikenda, praznih, odsutTypes };
    });
  ...
  const yearlyStats = useMemo(() => {
    ...
    return workers.filter(w => w.status === 'aktivan').map(w => {
      const months = Array.from({length:12}, (_,mi) => {
        ...
        return { radnih, odsutnih, vikenda, praznih, odsutTypes };
      });
      const total = months.reduce((a,m)=>({
        radnih:a.radnih+m.radnih, odsutnih:a.odsutnih+m.odsutnih,
        vikenda:a.vikenda+m.vikenda, praznih:a.praznih+m.praznih
      }),{radnih:0,odsutnih:0,vikenda:0,praznih:0});
  ...
  const singleWorkerData = useMemo(() => {
    ...
    const months = Array.from({length:12}, (_,mi) => {
      ...
      return { mi, days: daysList, radnih, odsutnih, vikenda, praznih, odsutTypes };
    });
    const total = months.reduce((a,m)=>({
      radnih:a.radnih+m.radnih, odsutnih:a.odsutnih+m.odsutnih,
      vikenda:a.vikenda+m.vikenda, praznih:a.praznih+m.praznih
            ...
            <div key={w.id} style={{background:'var(--surface)',border:`1px solid ${catBorder}`,borderLeft:`4px solid ${catColor}`,borderRadius:6,marginBottom:'0.4rem',overflow:'hidden'}}>
              ...
              <div style={{padding:'0.25rem 0.35rem 0.3rem',display:'grid',gridTemplateColumns:`repeat(${daysInMonth}, 1fr)`,gap:'1.5px'}}>
                {days.map(d => {
                  ...
                  return (
                    <div key={d} title={`${d}. ${entry?.type==='rad'?(cat?.short||'Rad')+' · '+entry.jobType : entry?.oType || (wknd?'vikend':'')}`}
                      style={{
                        ...
                        <tbody>
                          <tr>
                            {mo.days.map(({d,iso,wknd,entry}) => {
                              ...
                              return <td key={d} title={entry?.type==='rad'?entry.jobType:entry?.oType||''} style={{padding:'0.2rem 0.1rem',border:`1px solid ${border}`,textAlign:'center',background:bg}}>{content}</td>;
                            })}
                          </tr>
                        </tbody>
                      </table>
